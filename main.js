...
function gameOver() {
  ...
  const gameOverMessage = `A játéknak vége! Végső pontszám - <span class = 'badge'>${score}</span> Klikkelj a "Játsszunk" gombra, hogy újrakezdjük a játékot`;
...
function updateScore() {
  ...
  updateStatusElement(
    ...
    "block",
    primaryColor,
    `Pont <span class='badge'>${score}</span>`
  );
}
...
function checkForIncompleteGame() {
  ...
  if (serializedGameObj) {
    ...
    if (gameObj.round >= maxRounds) {
      ...
      if (confirm("Szeretnéd folytatni a legutóbbi játékodat?")) {
        score = gameObj.score;
...
function initializeNewGame() {
  ...
  updateStatusElement(
    ...
    "block",
    primaryColor,
    `Kör <span class='badge'>${roundNum}</span>`
  );
}
function startRound() {
  initializeNewRound();
  ...
  function shuffle() {
    ...
    if (shuffleCount == 500) {
      ...
      updateStatusElement(
        ...
        "block",
        primaryColor,
        "Klikkelj arra a kártyára, amelyről azt gondolod, hogy Petőfi van rajta..."
      );
    } else {
      shuffleCount++;
    }
